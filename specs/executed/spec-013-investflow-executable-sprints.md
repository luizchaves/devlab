# Spec 013 — InvestFlow: Executable Sprints with Automated Tests
...
The InvestFlow trail (`src/content/docs/courses/npm/practice/investflow/`, 9 pages) was
...
| ------ | ---- | ------------------- | ----- |
| 1 · Front estático | `front-static.mdx` | `investflow-static` (6 HTML, CSS, docs) | none |
...
1. One executable project per sprint, cumulative by construction, under
   `examples/courses/npm/projects/`: `investflow-static` (exists), `investflow-auth`,
   `investflow-database`, `investflow-edge-quotes`, `investflow-storage`,
   `investflow-analytics`. Each has `package.json`, `README.md`, `supabase/` (config,
...
convention: `investflow-<sprint>` copies the previous folder plus the sprint delta, so
...
### Phase 1 — Sprint 2 (`investflow-auth`)

1. Copy `investflow-static` → `investflow-auth`; add `@supabase/supabase-js`, Vitest, jsdom,
...
5. `.devcontainer/investflow-auth` (Docker-in-Docker feature for the local stack).
...
### Phase 2 — Sprint 3 (`investflow-database`)
...
### Phase 3 — Sprint 4 (`investflow-edge-quotes`)
...
### Phase 4 — Sprint 5 (`investflow-storage`)
...
### Phase 5 — Sprint 6 (`investflow-analytics`)
...
### Phase 6 — Sprint 7 (`investflow-origins`)
...
- [x] Phase 1 — Sprint 2 (`investflow-auth`): 15 Vitest (9 unit + 6 integration) and 4 Playwright tests green against `supabase start`; `auth.mdx` rewritten with `<SourceCode>` cuts and real outputs. Found and fixed during the work: the local stack grants no `select`/`update` on new tables by default, and a whole-row `update` policy would have let any account promote itself to `admin`; the migration now grants `select, update (full_name)` to `authenticated`.
- [x] Phase 2 — Sprint 3 (`investflow-database`): 36 Vitest (18 unit + 18 integration) and 8 Playwright tests green; `brokers`, `assets.issuer` and `asset.html` (TK03-10, TK03-11) added per the backlog revision of `e1336dfd`; every RLS policy has an integration test with two accounts.
- [x] Phase 3 — Sprint 4 (`investflow-edge-quotes`): 48 Vitest (24 unit + 24 integration, the latter `fetch`ing the local edge runtime) and 9 Playwright tests green; providers `fake` / `yahoo` / `brapi`; `pg_cron` scheduling via a `security definer` function whose `EXECUTE` is revoked from `PUBLIC` (found by the test).
- [x] Phase 4 — Sprint 5 (`investflow-storage`): 63 Vitest (31 unit + 32 integration against the local Storage) and 11 Playwright tests green; the aggregate stats function needed the role check outside the `count(*)` (found by the test).
- [x] Phase 5 — Sprint 6 (`investflow-analytics`): 79 Vitest (39 unit + 40 integration) and 14 Playwright tests green; the integration test caught an arithmetic error in the page's reference spreadsheet (February contribution is 3,400, not 3,200), now fixed on the page and recorded there.
- [x] Phase 6 — Sprint 7 (`investflow-origins`): 92 Vitest (48 unit + 44 integration) and 16 Playwright tests green; squarified treemap and line chart as pure modules with SVG rendering, no chart library.
- [x] Phase 7 — Closing
...
| ------- | ------ | ----- |
| Executable sprint projects | 1 (`investflow-static`) | **7** (`investflow-static` … `investflow-origins`), one folder per sprint |
