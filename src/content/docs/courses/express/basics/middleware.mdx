...
Middlewares executam na ordem em que são registrados. O diagrama a seguir ilustra a cadeia de responsabilidade (*pipeline*) de requisição e resposta do Express:

<Mermaid title="Cadeia de Responsabilidade e Pipeline de Middlewares no Express.js">
{`
flowchart LR
  A["Cliente HTTP (Request)"] --> B["1. express.json()<br/>(Parser do Body)"]
  B -->|"next()"| C["2. logger<br/>(Log da Requisição)"]
  C -->|"next()"| D["3. userRouter<br/>(Controlador de Rota)"]
  D -->|"Responde ou next(err)"| E["4. errorHandler / Resposta HTTP"]
  E --> F["Cliente HTTP (Response)"]
`}
</Mermaid>

Middlewares executam na ordem sequencial de registro no `app.use()`:
...
[MVC](../../architecture/mvc/) — juntando rotas, controllers, models e middlewares.
