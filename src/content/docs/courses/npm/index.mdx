...
<Mermaid title="Arquitetura do Ecossistema de Pacotes">
{`
flowchart TD
  subgraph G1["1. Gerenciamento e Resolução"]
    direction LR
    A["Manifesto<br/>package.json"] --> B["Gerenciador<br/>npm / pnpm / Yarn / Bun"]
    B --> C["Registro Remoto<br/>npmjs.com / jsr.io"]
    C --> D["Dependências Locais<br/>node_modules / store"]
  end

  subgraph G2["2. Ferramentas e Build"]
    direction LR
    E1["Qualidade e Testes<br/>Biome / Vitest / Playwright / Husky"]
    E2["Build & Dev Server<br/>Vite"]
  end

  subgraph G3["3. Aplicação e Bibliotecas"]
    direction TB
    subgraph G3A["Front-end e Interface"]
      direction LR
      F1["Interface & UI<br/>Tailwind / Primitivas / Lucide"]
      F2["Frameworks & SSR<br/>React / Vue / Next.js / Astro"]
    end
    subgraph G3B["Dados e Integração"]
      direction LR
      F3["APIs & Validação<br/>Axios / Zod / Valibot / TanStack"]
      F4["Banco & BaaS<br/>Prisma / Drizzle / Supabase"]
    end
    subgraph G3C["Back-end e IA"]
      direction LR
      F5["Servidor & Auth<br/>Express / Fastify / Hono / Clerk"]
      F6["IA & LLMs<br/>Vercel AI SDK / OpenAI"]
    end
  end
