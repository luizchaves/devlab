...
describe('renda fixa pelo valor e rendimento', () => {
  it('CA14.1, CA14.2, CA14.4 — a aplicação grava o valor como quantidade com preço 1 e guarda o rendimento com o indexador', async () => {
    const { ana } = await createAccounts();
    actAs(ana);
    const { asset } = await createAsset({ ticker: 'CDB-INTER', name: 'CDB Inter', category: 'renda_fixa' });

    const response = await postTransaction(jsonRequest('POST', { assetId: asset.id, type: 'buy', quantity: 1250.5, price: 1, transactionDate: '2026-01-10', yieldRate: 110, yieldIndex: 'cdi' }));
    const { transaction } = await response.json();
    expect(response.status).toBe(201);
    expect(transaction).toMatchObject({ quantity: 1250.5, price: 1, yieldRate: 110, yieldIndex: 'cdi' });

    const { asset: reloaded } = await (await getAsset(new Request('http://localhost'), params(asset.id))).json();
    expect(reloaded.transactions[0]).toMatchObject({ yieldRate: 110, yieldIndex: 'cdi' });
    ...
    expect(without.transaction).toMatchObject({ yieldRate: null, yieldIndex: 'fixed' });
  });
});
