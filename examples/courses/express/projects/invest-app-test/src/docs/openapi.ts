...
/** O banco guarda o caminho publico; o binario vive em `public/imgs/profile/`. */
const image = {
  type: 'object',
  properties: {
    id: { type: 'string', format: 'uuid' },
    path: { type: 'string', example: '/imgs/profile/2c140192-avatar.png' },
...
export const openapi = {
  ...
  tags: [
    ...
    { name: 'Auth', description: 'Login e emissao de token' },
    { name: 'Images', description: 'Avatar do perfil' },
  ],
  components: {
    securitySchemes: {
  ...
  // faz o Swagger UI exibir o botao "Authorize" e enviar o cabecalho no Try it out.
  security: [{ bearerAuth: [] }],
  paths: {
    '/investments': {
      get: {
          ...
          200: {
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  properties: { auth: { type: 'boolean' }, token: { type: 'string' } },
                },
              },
            },
          },
    ...
    '/users/image': {
      post: {
        tags: ['Images'],
        summary: 'Envia o avatar do perfil',
        requestBody: {
          required: true,
          content: {
            'multipart/form-data': {
        ...
        responses: {
          201: {
            description: 'Avatar gravado. A resposta traz o caminho publico, nao o binario.',
            content: { 'application/json': { schema: image } },
          },
          400: { description: 'Arquivo ausente, tipo nao permitido ou acima de 2 MB.' },
          401: unauthorized,
        },
      },
      put: {
        tags: ['Images'],
        summary: 'Substitui o avatar do perfil',
        requestBody: {
          required: true,
          content: {
            'multipart/form-data': {
        ...
        responses: {
          200: {
            description: 'Avatar substituido.',
            content: { 'application/json': { schema: image } },
          },
          400: { description: 'Arquivo ausente, tipo nao permitido ou acima de 2 MB.' },
          401: unauthorized,
