...
export function index(req: Request, res: Response) {
  ...
  res.json(Task.findAll(query));
}

export function show(req: Request, res: Response) {
  const { params } = validated(req, showTaskSchema);
  const task = Task.findById(params.id);
...
export function store(req: Request, res: Response) {
  ...
  if (Task.findByTitle(body.title)) {
    throw new HttpError(409, 'Ja existe uma tarefa com esse titulo');
  }

  res.status(201).json(Task.create(body));
}
// #endregion
...
export function update(req: Request, res: Response) {
  ...
  const task = Task.update(params.id, body);
