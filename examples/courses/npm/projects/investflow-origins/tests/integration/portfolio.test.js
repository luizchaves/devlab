...
describe.skipIf(!hasStack)('Sprint 3 contra a stack local', () => {
  ...
  it('RF03.4 / ajuste: edita e exclui transacao recalculando a posicao', async () => {
    ...
    // Edita a transacao
    const { error: updateErr } = await ana.client
      .from('transactions')
      .update({ quantity: 60, price: 25 })
      .eq('id', tx.id);
    expect(updateErr).toBeNull();

    const { data: updatedTx } = await ana.client
      .from('transactions')
      .select('quantity, price')
      .eq('id', tx.id)
      .single();
    expect(Number(updatedTx.quantity)).toBe(60);
    expect(Number(updatedTx.price)).toBe(25);

    // Exclui a transacao
    const { error: deleteErr } = await ana.client.from('transactions').delete().eq('id', tx.id);
  ...
  it('CA03.5: update e delete cruzados de transacoes nao alcancam nenhuma linha', async () => {
    const { data: anaTxs } = await ana.client
      .from('transactions')
      .select('id')
      .eq('asset_id', petr4.id);
    const targetId = anaTxs[0].id;
    ...
    const upd = await bruno.client
      ...
      .eq('id', targetId)
      .select();
    const del = await bruno.client.from('transactions').delete().eq('id', targetId).select();
