...
as $$
  ...
  select coalesce((select quantity from latest_update), 0)
       + coalesce((
           select sum(case when type = 'buy' then quantity when type = 'sell' then -quantity else 0 end)
           from public.transactions
           where asset_id = p_asset_id
             and transaction_date <= p_date
             and type in ('buy', 'sell')
             and transaction_date > coalesce((select transaction_date from latest_update), '-infinity'::date)
         ), 0);
$$;
...
with month_value as (
  select a.user_id,
         ...
         sum(public.position_at(a.id, (date_trunc('month', q.quote_date) + interval '1 month - 1 day')::date) * q.price * (case when a.currency = 'USD' then public.get_usd_rate(date_trunc('month', q.quote_date)::date) else 1.0 end)) as value
...
month_flow as (
  select t.user_id,
         ...
         sum((case when t.type = 'buy' then t.quantity * t.price when t.type = 'sell' then -t.quantity * t.price else 0 end) * (case when a.currency = 'USD' then public.get_usd_rate(date_trunc('month', t.transaction_date)::date) else 1.0 end)) as net_flow
