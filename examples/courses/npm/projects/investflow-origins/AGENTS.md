...
Instruções para agentes de IA que trabalham no projeto **InvestFlow**.
...
6. **Seeds sem dados confidenciais**: `supabase/seed.sql` e `scripts/seed-investments.mjs` devem conter apenas dados demonstrativos públicos. Carteiras reais, saldos privados e históricos financeiros confidenciais ficam fora do repositório.
7. **Idioma dos Identificadores**: Identificadores de código (funções, variáveis, colunas SQL, tipos) em inglês; interface do usuário, comentários didáticos e documentação em português do Brasil.
8. **Ícones de Interface**: Quando for necessário usar ícones na interface, utilize a biblioteca Lucide, preservando rótulos acessíveis e sem criar SVGs manuais para ícones comuns.
...
| `pnpm db:start` / `pnpm db:stop` | Sobe e derruba a stack local do Supabase (Docker) |
