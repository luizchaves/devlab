...
async function renderEvolution() {
  const { data } = await portfolioEvolution(id);
  const rows = totalsByMonth(data ?? []);
  document
    .querySelector('[data-evolution]')
    .replaceChildren(
      rows.length
        ? renderLineChart(toSeries(rows), { format: formatBRL })
        : 'Sem cotação registrada para este ativo ainda.'
    );
}
// #endregion
// #endregion
