...
select a.user_id,
       ...
       public.position_at(a.id, current_date) * coalesce(a.current_price, 0) as value
...
select m.user_id,
       ...
       m.month,
       coalesce((
         select sum(case when t.type = 'buy' then t.quantity * t.price else -t.quantity * t.price end)
         from public.transactions t
         where t.asset_id = m.asset_id and t.transaction_date <= (m.month + interval '1 month - 1 day')::date
       ), 0) as invested,
       public.position_at(m.asset_id, m.last_quote_date)
         * (select price from public.quotes_history where asset_id = m.asset_id and quote_date = m.last_quote_date) as value
from months m;
