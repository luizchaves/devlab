...
async function create({ name, email, password }: UserInput): Promise<PublicUser> {
  ...
  return await prisma.user.create({ data: { name, email, password: hash }, select });
}
...
/** A unica funcao que devolve o hash — usada so pelo login da etapa 10. */
...
export default { create, readById, readByEmailWithPassword };
