...
/**
 * Atualiza o cartao no lugar, sem recarregar a lista inteira: o servidor ja
 * mandou tudo que mudou.
 */
function applyPing({ hostId, success, latency }) {
  const card = document.querySelector(`#host-${hostId}`);

  if (!card) return;

  card.querySelector('.status-dot').className = `status-dot ${
    success ? 'status-online' : 'status-offline'
  }`;

  card.querySelector('.host-latency').innerText = formatLatency(latency);

  card.querySelector('.latency-bar').style.setProperty('--latency', latencyRatio(latency));
}

function loadLiveUpdates() {
  subscribe(applyPing).catch((error) => console.error('Fluxo interrompido', error));
}
