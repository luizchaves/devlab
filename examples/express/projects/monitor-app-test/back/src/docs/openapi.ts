...
export const openapi = {
  ...
  tags: [
    ...
    { name: 'Users', description: 'Cadastro de contas' },
    { name: 'Auth', description: 'Login e emissão de token' },
    { name: 'Events', description: 'Fluxo de medições em tempo real (SSE)' },
  ],
  paths: {
    '/hosts': {
      get: {
    ...
    '/users': {
      post: {
        ...
        responses: {
          ...
          400: validationError,
          409: { description: 'Já existe uma conta com este e-mail.' },
        },
      },
    },
    '/signin': {
    ...
    '/events': {
      get: {
        tags: ['Events'],
        summary: 'Assina o fluxo de medições do usuário (Server-Sent Events)',
        description:
          'A resposta é um `text/event-stream` que não termina: cada medição chega como um bloco `event: ping`.',
        responses: {
          200: {
            description: 'Fluxo aberto.',
