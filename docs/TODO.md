...
- [ ] **[TASK-033] TaskAPI — ajustes e páginas pendentes:**
  ...
  - [x] `[TASK-033.5]` Validar cookies com Zod na etapa de autenticação (spec: `specs/executed/spec-012-zod-headers-cookies-validation.md`)
- [ ] **[TASK-034] Novas validações de segurança:**
  - [x] `[TASK-034.1]` Validar o JWT (assinatura, expiração e claims) na etapa de autenticação da TaskAPI (spec: `specs/executed/spec-012-zod-headers-cookies-validation.md`)
...
- [ ] **[TASK-016] Estruturação e Documentação dos Projetos:**
  ...
  - [x] `[TASK-016.8]` InvestFlow executável: um projeto por sprint (`investflow-static` a `investflow-origins`), stack local do Supabase, e cada task validada por testes de unidade (Vitest), integração (SDK real) e E2E (Playwright); páginas com `<SourceCode>` real e saídas reais (spec: `specs/executed/spec-013-investflow-executable-sprints.md`)
  - [x] `[TASK-016.9]` Deploy do front do InvestFlow na Vercel (`vercel.json`, variáveis públicas, dica de IA com o MCP da Vercel)
  - [x] `[TASK-016.10]` Rastreabilidade do InvestFlow para as features de `investflow-origins`: trilha reestruturada em 11 sprints (8. lançamentos e cotação manual, 9. proventos e movimentações, 10. dólar e cripto, 11. perfil e experiência), US17–US32 com 64 critérios novos numerados por sprint, cada um no nome de um teste (216 Vitest, 33 Playwright), e quatro defeitos corrigidos pelo caminho (spec: `specs/executed/spec-014-investflow-backlog-traceability.md`)
  - [x] `[TASK-016.11]` InvestFlow React: os requisitos RF01–RF22 do InvestFlow vanilla reconstruídos em `examples/courses/react/investflow-react` (Next.js, NextAuth, Prisma no Postgres do Supabase, TanStack Query/Table, Zustand, Base UI, CVA, Tailwind CSS v4, Vitest em três camadas e Playwright, mais o RNF07 de design responsivo), 123 Vitest + 2 de build + 36 Playwright, e a seção Na prática › InvestFlow (React) do guia com 11 páginas (spec: `specs/executed/spec-015-investflow-react.md`)
